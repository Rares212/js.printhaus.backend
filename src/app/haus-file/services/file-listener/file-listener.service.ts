...
import { streamToBuffer } from '@src/app/common/util/stream.util';
...
export class FileListenerService implements OnModuleInit, OnModuleDestroy {
    ...
    async handleChange(change) {
        if ((change.operationType === 'insert' ||
            ...
            if (fileInfo.title && fileInfo.title.endsWith('-thumbnail')) {
                return;
            }
    ...
    private async generateThumbnail(image: Buffer, fileInfo: FileInfo) {
        ...
        thumbnailFileInfo.title = `${fileInfo.title}-thumbnail`;
